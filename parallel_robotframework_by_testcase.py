...
testcases = []
def get_testname_by_file(testfile, include_tag='core', exclude_tag='regression'):
...
def get_testname_by_folder(folder="/Users/jollychang/Work/shire/webtests/", include_tag='core', exclude_tag=''):
    for root, dirs, files in os.walk(folder):
        for f in files:
            if bool(re.match('test\_.*\.txt$', f)):
                absoulte_path = os.sep.join([root, f])
                get_testname_by_file(absoulte_path, include_tag, exclude_tag)
...
if __name__=="__main__":
    folder='/Users/jollychang/Work/shire/'
    get_testname_by_folder(folder=folder, include_tag='core', exclude_tag='')
